							...
							<form class="pricing" action="search" method="get">
								<select name="pricing" onchange='this.form.submit()'>
									...
									<option
										...
										value="under50">under 10.000.000</option>
									<option
										<c:if test="${pricing eq '50to70'}">
										selected="selected"
										</c:if>
										value="50to70">10.000.000 to 50.000.000</option>
									<option
										<c:if test="${pricing eq 'greaterthan70'}">
										selected="selected"
										</c:if>
										value="greaterthan70">greater than 50.000.000</option>
										...
										<div class="products">
											...
											<div class="thumbnail">
												<a href="product-details?productId=${product.productId}"><img
													style="width: 90%;"
													src="<c:url value="/resource/client/images/products/image/${product.image}" />" alt="Product Name"></a>
											</div>
											...
											<c:if test="${product.saleDTO.salePercent == 0}">
												...
												<span class="price">${Math.round(product.price - (product.price
													* product.saleDTO.salePercent / 100))} đ</span>
											</c:if>

											<c:if test="${product.saleDTO.salePercent != 0}">
												<span class="price"
													style="font-size: 15px; color: black; text-decoration: line-through; margin-bottom: 0px; margin-top: -5px;">${Math.round(product.price)} đ</span>
												<span class="price">${Math.round(product.price - (product.price
													* product.saleDTO.salePercent / 100))} đ</span>
											</c:if>
